<?php
/* confirm_email.php - BetSter project (22.05.06)
...
if ((isset($_GET['cn']) && isset($_GET['un']) && isset($_GET['al']))) {
	$conf_num = $_GET['cn'];
	$xusername = $_GET['un'];
	$email = base64_decode($_GET['al']);
	$xuser = $db_mapper->getUser($xusername);
	$conf_num_db = $db_mapper->getUserConfNum($xusername);
	if (($conf_num_db == $conf_num) && (checkEmail($email))) {
		$xuser->setEmail($email);
		$logger->writeLog($xusername, _EMAIL_CHANGED.$email);
		$mainhtml = replace("Message1", _CHANGE_EMAIL_SUC, $mainhtml);
	}
	else {
		$mainhtml = replace("Message1", _CHANGE_EMAIL_FAIL, $mainhtml);
	}
}
else {
